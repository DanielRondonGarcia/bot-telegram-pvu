...
def uno():
    ...
    for data_ in data["config"]:
        if app.get_user_active(data_["id"]) == True:
            bot.send_message(data_["id"], scrapping.messsage_info_general(data_["token"]), parse_mode='HTML')   
    ...
    for data_ in data["config"]:
        if app.get_user_active(data_["id"]) == True:
            ...
            if not alerta:
                ...
                for i in alerta:
                    print(i)
                    bot.send_message(data_["id"], i, parse_mode='HTML')
def tres():
    for data_ in data["config"]:
        if app.get_user_active(data_["id"]) == True:
...
def cuatro():
    for data_ in data["config"]:      
        if app.get_user_active(data_["id"]) == True:          
            bot.send_message(data_["id"], scrapping_tree.messsage_info_tree_general(data_["token"]), parse_mode='HTML')
...
def send_welcome(message):
    bot.reply_to(message,f"<b> Espera un momento...</b>",parse_mode='HTML')
    ...
    if app.get_user(private_id) == False:            
        bot.send_message(private_id,f"Usuario: <code>{str(username)}</code> NO se encuentra registrado",parse_mode='HTML')
    else:
        if flag_data_json:
            ...
            bot.send_message(private_id,f"Usuario: <code>{str(username)}</code> ya se encuentra registrado",parse_mode='HTML')            
        ...
        if data_["id"] == str(private_id):   
            if app.get_user_active(private_id) == True:         
                ...
                if not alerta:
                    ...
                    print(alerta)
                    for i in alerta:
                        bot.send_message(private_id, i, parse_mode='HTML')
            else:
                bot.send_message(private_id, 'Usuario inactivo', parse_mode='HTML')

        else:
            bot.send_message(private_id, 'No has iniciado el bot', parse_mode='HTML')
...
def send_welcome(message):
    ...
    for data_ in data["config"]:   
        if data_["id"] == str(private_id):
            if app.get_user_active(private_id) == True:
                bot.send_message(private_id, scrapping_tree.messsage_info_tree_general(data_["token"]), parse_mode='HTML')
            else:
                bot.send_message(private_id, 'Usuario inactivo', parse_mode='HTML')
        else:
            bot.send_message(private_id, 'No has iniciado el bot', parse_mode='HTML')
...
def send_welcome(message):
    ...
    for data_ in data["config"]:   
        if data_["id"] == str(private_id):
            if app.get_user_active(private_id) == True:
                bot.send_message(private_id, scrapping.messsage_info_general(data_["token"]), parse_mode='HTML')
            else:
                bot.send_message(private_id, 'Usuario inactivo', parse_mode='HTML')
        else:
            bot.send_message(private_id, 'No has iniciado el bot', parse_mode='HTML')
...
def send_welcome(message):
    ...
    json_ = json.loads(json_)
    private_id = json_["from_user"]["id"]
    bot.send_message(private_id, '<a href="t.me/utilitiesPVU_bot">utilities Plant Vs Undead</a>',parse_mode='HTML')
    texto=''
    ...
    if app.get_user_active(private_id) == True:     
        for command_ in commands:
            ...
            markup.row(command_)
        bot.send_message(private_id, texto,reply_markup=markup,parse_mode='HTML')
    else:
        bot.send_message(private_id, 'No te encuentras registrado',reply_markup=markup,parse_mode='HTML')        
    bot.send_message(private_id, 'Debes inciar el bot para visualizar tu información /start para inciarlo',reply_markup=markup,parse_mode='HTML')
